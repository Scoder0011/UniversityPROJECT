    ...
    try:
        ...
        for p in temp_files:
            try:
                ...
                pass
                def convert_to_pdf_for_preview(file_path, file_type):
    """Convert any supported file to PDF for preview"""
    if file_type == 'pdf':
        return file_path
    
    temp_pdf = os.path.join(TEMP_DIR, f"preview_{datetime.now().timestamp()}.pdf")
...
# NEW HELPER: Generate page preview thumbnails
def generate_page_previews(pdf_path, max_pages=50):
    """Generate base64 encoded thumbnails for PDF pages"""
    try:
        ...
        for page_num in range(min(len(doc), max_pages)):
            page = doc[page_num]
            # Render at lower resolution for faster preview
...
# NEW ENDPOINT: Get page count and previews
@app.route('/get-page-info', methods=['POST'])
def get_page_info():
    """Get page count and preview thumbnails for uploaded files"""
    if 'files' not in request.files:
        return {'error': 'No files uploaded'}, 400
    ...
    try:
        ...
        for file in files:
            ...
            pdf_path = convert_to_pdf_for_preview(temp_path, file_type)
            ...
            # Generate previews
            previews = generate_page_previews(pdf_path)
...
# NEW ENDPOINT: Single file page extraction
@app.route('/extract-pages-single', methods=['POST'])
def extract_pages_single():
    """Remove selected pages from a single file"""
    ...
    try:
        pages_to_remove = [int(p.strip()) for p in pages_to_remove.split(',') if p.strip()]
    except:
        return {'error': 'Invalid page numbers'}, 400
    ...
    try:
        ...
        # Write output
        output_filename = f"extracted_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        output_path = os.path.join(TEMP_DIR, output_filename)
        ...
        import traceback
        print("ERROR in /extract-pages-single:", traceback.format_exc())
        return {'error': str(e)}, 500
...
# NEW ENDPOINT: Mix multiple files and extract pages
@app.route('/extract-pages-mix', methods=['POST'])
def extract_pages_mix():
    """Mix multiple files, remove selected pages, return single PDF"""
    if 'files' not in request.files:
        return {'error': 'No files uploaded'}, 400
    
    files = request.files.getlist('files')
    
    # Get removal instructions: {"file_0": [1,3,5], "file_1": [2,4]}
    try:
        removal_data = json.loads(request.form.get('removal_data', '{}'))
    except:
        return {'error': 'Invalid removal data'}, 400
    ...
    try:
        ...
        for idx, file in enumerate(files):
            ...
            filename = secure_filename(file.filename)
            temp_path = os.path.join(TEMP_DIR, f"{datetime.now().timestamp()}_{idx}_{filename}")
            file.save(temp_path)
            temp_files.append(temp_path)
            ...
            # Get pages to remove for this file
            file_key = f"file_{idx}"
            pages_to_remove = removal_data.get(file_key, [])
            ...
            # Write filtered file
            filtered_pdf = os.path.join(TEMP_DIR, f"filtered_{idx}_{datetime.now().timestamp()}.pdf")
            temp_files.append(filtered_pdf)
            
            with open(filtered_pdf, 'wb') as f:
                writer.write(f)
            
            # Add to merger
            merger.append(filtered_pdf)
        
        # Write final merged PDF
        output_filename = f"mixed_extracted_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        output_path = os.path.join(TEMP_DIR, output_filename)
        ...
        import traceback
        print("ERROR in /extract-pages-mix:", traceback.format_exc())
        return {'error': str(e)}, 500
